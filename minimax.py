...
def minimax_method(current_color):
    def get_legal_moves():
        moves = []
        for pos, info in hexagon_board.items():
            if not info['selected'] and pos != (0, 0):
                moves.append((pos, current_color))  # own stone
                moves.append((pos, 'gray'))         # neutral stone
        return moves

    def minimax(depth, maximizing, alpha, beta, color):
        if depth == 0 or check_all_hexes_selected():
            return evaluate_board_for(current_color, hexagon_board), None
        ...
        if maximizing:
            ...
            for move in legal_moves:
                pos, stone_color = move
                backup = hexagon_board[pos].copy()
                hexagon_board[pos]['selected'] = True
                hexagon_board[pos]['color'] = stone_color
                eval_score, _ = minimax(depth - 1, False, alpha, beta, opponent_color)
                hexagon_board[pos] = backup
            ...
            for move in legal_moves:
                ...
                hexagon_board[pos]['selected'] = True
                hexagon_board[pos]['color'] = stone_color
                eval_score, _ = minimax(depth - 1, True, alpha, beta, opponent_color)
                hexagon_board[pos] = backup
    ...
    _, best_move = minimax(depth=2, maximizing=True, alpha=float('-inf'), beta=float('inf'), color=current_color)
    if best_move:
        (pos_x, pos_y), stone_color = best_move
        return pos_x, pos_y, stone_color
    else:
        legal_moves = get_legal_moves()
        if legal_moves:
            (pos_x, pos_y), stone_color = random.choice(legal_moves)
            return pos_x, pos_y, stone_color
        return None
...
def main(black_player, white_player):
    ...
    while not check_all_hexes_selected():
        ...
        if (current_turn == 'black' and black_player_type == "AI") or (current_turn == 'white' and white_player_type == "AI"):
            ...
            pos_x, pos_y, color = minimax_method(current_turn)
